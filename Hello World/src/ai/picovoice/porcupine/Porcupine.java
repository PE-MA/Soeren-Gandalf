...
/**
 * Java binding for Porcupine wake word engine. It detects utterances of given
 * keywords within an incoming stream of audio in real-time. It processes
 ...
 * {@link #getFrameLength()}. The incoming audio needs to have a sample rate
...
public class Porcupine {
	 ...
	 *                      be a number within [0, 1]. A higher sensitivity results
	 *                      in fewer misses at the cost of increasing the false
	 *                      alarm rate.
	 * @throws PorcupineException if there is an error while initializing Porcupine.
	 */
	public Porcupine(String libraryPath, String modelPath, String[] keywordPaths, float[] sensitivities)
	 ...
	 *            be 16-bit linearly-encoded. Porcupine operates on single-channel
	 *            audio.
	 * @return Index of observed keyword at the end of the current frame. Indexing
	 *         is 0-based and matches the ordering of keyword models provided to the
	 *         constructor. If no keyword is detected then it returns -1.
	 * @throws PorcupineException if there is an error while processing the audio
	 *                            frame.
	 */
	...
	/**
	 * Builder for creating an instance of Porcupine with a mixture of default
	 * arguments
	 */
	public static class Builder {
		...
		/**
		 * Validates properties and creates an instance of the Porcupine wake word
		 * engine.
		 *
		...
		public Porcupine build() throws PorcupineException {
			...
			if (modelPath == null) {
				if (Utils.isResourcesAvailable()) {
					...
					throw new PorcupineException(
							new IllegalArgumentException("Default model unavailable. Please provide a "
			...
			if (this.keywordPaths != null && this.keywords != null) {
				throw new PorcupineException(
						new IllegalArgumentException("Both 'keywords' and 'keywordPaths' were set. "
			...
			if (this.keywordPaths == null) {
				...
				if (Utils.isResourcesAvailable()) {
					...
					throw new PorcupineException(
							new IllegalArgumentException("Default keywords unavailable. Please provide "
			...
			if (sensitivities.length != keywordPaths.length) {
				throw new PorcupineException(new IllegalArgumentException(
						String.format("Number of keywords (%d) " + "does not match number of sensitivities (%d)",
