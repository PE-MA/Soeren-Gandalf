			...
			if (arch.equals("arm") || arch.equals("aarch64")) {
				...
				default:
					throw new RuntimeException(String.format(
							"Execution environment not supported. " + "Porcupine Java does not support CPU Part (%s).",
							cpuPart));
				}
			}
			return "linux";
		...
		if (arch.equals("amd64") || arch.equals("x86_64")) {
			...
			default:
				throw new RuntimeException(String
						.format("Platform architecture with CPU Part (%s) is not supported by Porcupine.", cpuPart));
			}
		} else {
			throw new RuntimeException(
					String.format("Platform architecture (%s) is not supported by Porcupine.", arch));
		}
	}

	private static String getCpuPart() throws RuntimeException {
		try {
			return Files.lines(Paths.get("/proc/cpuinfo")).filter(line -> line.startsWith("CPU part"))
...
System.out.println(keywordFileDir);
	...
	public static String getPackagedLibraryPath() {
		...
		case "raspberry-pi":
		case "linux":
			return RESOURCE_DIRECTORY.resolve("lib/java").resolve(ENVIRONMENT_NAME).resolve(ARCHITECTURE)
